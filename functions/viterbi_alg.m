function [max_ind,delta,delta2,prb]=viterbi_alg(obs,Mu,Cov,prior,trans)
...
for t=2:length(obs)
    for state=1:p
        ...
        delta2(state,t)=max(delta2(:,t-1).*trans(:,state))*pdf_vals(state,t);
        [~,max_gamm]=max(delta(:,t-1)+log(trans(:,state)));
        prb(state,t)=delta2(:,t-1)'*trans(:,state);
